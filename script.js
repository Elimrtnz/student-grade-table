...
function addStudent(){
    if(processRequest===true){
        ...
        //updateDataOnDom(); --------------------------------- need to move after success response in addStudentToDB
        clearAddStudentForm();
...
function deleteFromDom(){
    ...
    var databaseName = student_array[rowIndex].name;
    //************************************************************************************************************deleteStudentFromDB(databaseId,databaseName);
    deleteStudentFromDB(databaseId,databaseName);
...
function addStudentToDB(students_obj){
    ...
    $.ajax( {
        ...
        success: function(response){
            alert("Added student : " + insert_data_object.name+" \n The above record has been added to the database.");
            //after successful response from the database update dom
            updateDataOnDom();
        },
        error: function (response) {
            alert("The student could not be added to the database, please check your inputs and try again.");
        },
        dataType: 'json',
        method: 'post',
    });
...
function deleteStudentFromDB(id,name){
    var delete_object = {
        student_id: id,
        student_name: name
    };
    ...
    $.ajax( {
        ...
        success: function(deleteResponse){
            alert("Deleted student : "+ delete_object.student_name + " \n The above record has been deleted from the database.");
            updateDataOnDom();
        },
        error: function (response) {
            alert("Sorry, There was an error connecting to the database. Please try again later.");
        },
        dataType: 'json',
        method: 'post',
    });
...
function updateStudentOnDB(update_StudentObj){
    ...
    $.ajax( {
        ...
        success: function(response){
            alert("Updated student : "+ update_data_object.updateName + " \n The above record has been updated in the database.");
            updateDataOnDom();
        },
        error: function (response) {
            alert("Sorry, There was an error connecting to the database. Please try again later.");
...
function sortList(){
    ...
    $.ajax( {
        ...
        success: function(sortResponse){
            ...
            alert("The data will be sorted by "+sortField+".");
            updateDataOnDom();
        },
        error: function (response) {
            alert("Sorry, there was an error sorting from the database. Please try again later.");
        },
        dataType: 'json',
        method: 'post',
    });
